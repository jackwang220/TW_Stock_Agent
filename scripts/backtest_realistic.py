...
def load_signals() -> dict[str, list[dict]]:
    ...
    for r in csv.DictReader(LLM_CSV.open(encoding="utf-8")):
        ...
        up = r.get("predicted_direction", "") == "up" and verdict != "REJECT"
        by_date.setdefault(d, []).append({
            "ticker": tk, "center": center, "conf": conf, "up": up, "verdict": verdict,
        ...
        if prev_d and prev_d in sigs:
            ...
            for s in sigs[prev_d]:
                ...
                if tk in held_tks or tk not in panel:
                    continue
                if cfg("trading.signals.require_pass", False) and s.get("verdict") != "PASS":
                    continue                                    # 只做 PASS(砍 WARN+REJECT)
                cl = closes_upto(tk, prev_d)
                if len(cl) < 25:
                    continue
                rb = rebound_signal(cl, avg_turnover_of(tk))
                if cfg("trading.signals.require_rebound", False) and not rb.get("fired"):
                    continue                                    # 反彈當選股層:沒觸發就不選
